...
def train(data_path:str, output_path:str, input_data:str, sample_index_file:str, 
    ...
    #Ra_zero = Y1[:,:,0].max().numpy()
    #Rh_zero = Y1[:,:,1].max().numpy()
    #print(Y1[:,:,0].max(),Y1[:,:,1].max(),Ra_zero,Rh_zero)
    ...
    seq_len=365
    model_version= pretrained_model #'recotest_v11_exp4.sav_step3'  
    ...
    model1.to(device)
    model_version= output_model #'recotest_v11_exp4_sample.sav_step4' 
    path_save = output_path+model_version
    print(model1)
    ...
    check_Rh2GPP_response = kgml_lib.check_Rh2GPP_response
    ...
    for epoch in range(maxepoch):
        ...
        for bb in range(int(train_n/bsz)):
            ...
            if epoch > 0:
                ...
                loss3 = check_Rh2GPP_response(model1, check_xset,Y1_scaler,device)
                optimizer.zero_grad()
                loss3.backward()
                optimizer.step()  
                with torch.no_grad():
        ...
        with torch.no_grad():
            ...
            loss3 = check_Rh2SOC_response(model1, check_xset,Y1_scaler,device)+\
                    check_Rh2GPP_response(model1, check_xset,Y1_scaler,device)
            loss = loss + loss3
            ...
            if np.mean(train_R2)> 0.999 or (epoch - best_epoch) > 5:
                break
        model1.train()
    endtime=time.time()
    ...
    print("Step 4 saved model is: ", path_fs)
