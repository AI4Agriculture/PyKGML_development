...
n2o_my_loss = kgml_lib.n2o_my_loss
my_loss = kgml_lib.my_loss
compute_r2 = kgml_lib.R2Loss()

class MLP:
    def __init__(self, input_path: str, output_path: str, input_data: str, output_model, dataset: Trad_ml_DataSet, sample_index_file: str = None, hidden_dim: int = 1024, num_layers: int = 4, dropout: float = 0.5):
        ...
        self.mini_batch = 4096
        self.hidden_dim = hidden_dim
        self.dropout = dropout
        self.num_layers = num_layers
    
       
        self.X_train = self.dataset.X_train.reshape(-1, self.dataset.n_X)
        self.Y_train = self.dataset.Y_train.reshape(-1, self.dataset.n_Y)
        self.X_val = self.dataset.X_val.reshape(-1, self.dataset.n_X)
        self.Y_val = self.dataset.Y_val.reshape(-1, self.dataset.n_Y)
    ...
    def build_model(self, input_dim, hidden_dim, output_dim, num_layers=4, dropout=0.5):
    ...
    def train_step(self, learning_rate=0.001, num_epochs=100): 
        ...
        model = self.build_model(input_dim, hidden_dim, output_dim, num_layers=num_layers, dropout=dropout)
    ...
    def test(self):
        ...
        model = self.build_model(input_dim, hidden_dim, output_dim, num_layers, dropout)  
        model = nn.DataParallel(model)
...
if __name__  == "__main__":
    ...
    ml_model = MLP(input_path=data_path, output_path=output_path, 
