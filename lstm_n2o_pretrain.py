...
import numpy as np
from dataset import Trad_ml_DataSet, Z_norm_reverse
...
class LSTM:
    def __init__(self, input_path: str, output_path: str, input_data: str, output_model, dataset: Trad_ml_DataSet, sample_index_file: str = None, num_layers: int = 2, hidden_dim: int = 64, dropout: float = 0.2):
    ...
    def build_lstm_model(self, input_dim, hidden_dim, output_dim, num_layers=2, dropout = 0.2):
        ...
        class LSTMNetwork(nn.Module):
            def __init__(self, input_dim, hidden_dim, output_dim, num_layers=num_layers, dropout=dropout):
                ...
                self.lstm_layer = nn.LSTM(input_dim, hidden_dim, num_layers, batch_first=True, dropout=dropout)
                self.relu1 = nn.ReLU()
            ...
            def forward(self, x, hidden_state=None):
                ## if the hidden state is none, initialize the hidden state
                ...
                output, hidden = self.lstm_layer(x, hidden_state) 
        ...
        return LSTMNetwork(input_dim, hidden_dim, output_dim, num_layers)
    ...
    def train_step(self, learning_rate=0.001, num_epochs=500):
        ...
        model = self.build_lstm_model(input_dim, hidden_dim, output_dim, num_layers, dropout)
        ...
        for epoch in range(num_epochs):
            ...
            for X_batch, Y_batch in self.train_loader:
                ...
                ## to cut the pytorch computation graph. the 18 years sequence ended and need to start from year 0 of a different batch
    ...
    def check_results(self, device, model_trained, check_xset, check_yset, num_year, starttime):
        ...
        with torch.no_grad():
            hidden_state = None
            for year in range(self.num_years):
                X_batch_year = check_xset[:, year, :, :].to(device)   # shape: (site, day, feature)
                Y_batch_year = check_yset[:, year, :, :].to(device)   # shape: (site, day, feature)
                ...
                outputs = outputs.squeeze(1)  # shape: (site, day, feature)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for X_batch, Y_batch in val_loader:
                ...
                loss, loss1, loss2, loss3, loss4, loss5 = n2o_my_loss(outputs, Y_batch_year, loss_weights)
    ...
    def vis_flux(self):
        ...
        for i in range(len(plot_names)):  
            ...
            check_set = self.check_y_all_true.reshape(-1, self.check_y_all_true.shape[-1]) # shape: (site, :)
            check_set_pred = self.check_y_all_pred.reshape(-1, self.check_y_all_pred.shape[-1]) # shape: (site, :)
...
if __name__  == "__main__":
    ...
    ml_model = LSTM(input_path=data_path, output_path=output_path, 
                        input_data=input_data, output_model=output_model, 
                        dataset=dataset, hidden_dim=hidden_dim, num_layers=num_layers, dropout=dropout)
